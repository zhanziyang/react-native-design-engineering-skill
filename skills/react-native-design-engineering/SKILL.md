...
Before doing any work, route deterministically:

- Strict review or findings request -> `review-react-native-animations`.
- Whole-project or multi-file audit or planning, or broad animation refactor -> `improve-react-native-animations`.
- Reverse lookup from a described effect to its canonical motion or gesture term -> `react-native-animation-vocabulary`.
- Apple-style interaction-principle reasoning -> `apple-design-react-native`.
